...
const ListProduct = () => {
    ...
    useEffect(() => {
        const ListKoi = async () =>{
            try{
                ...
                const response = await axios.get(`http://localhost:8080/auction/koi/productManage`);
    ...
    const searchNameKoi = (name) => {
        const searchTerm = String(name).toLowerCase(); // Ensure 'name' is a string
    ...
    const renderProductRows = (productList) => {
        return productList.map((product, index) => (
            <tr key={index}>
                ...
                <td><button onClick={() => handleEditProduct(product)}>Chọn</button></td>
            </tr>
        ));
    };
    return (
